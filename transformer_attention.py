...
class AttentionQKV(Model):
    ...
    def call(self, queries, keys, values, mask=None):
        ...
        # As defined in the Attention is all you need paper: https://arxiv.org/pdf/1706.03762.pdf
        key_dim = tf.cast(tf.shape(keys)[-1], tf.float32)
        if len(keys.shape) == 4:
            similarity = queries @ tf.transpose(keys, perm = [0, 1, 3, 2]) # Compute the similarity according to the QKV formula
        else:
            similarity = queries @ tf.transpose(keys, perm = [0, 2, 1])
        ...
        weights = tf.nn.softmax(masked_similarity / tf.sqrt(key_dim), axis = len(keys.shape) - 1) # Turn the similarity into a normalized output
...
class MultiHeadProjection(Model):
    ...
    def _split_heads(self, tensor):
        ...
        seq_length = tensor.shape.as_list()[1]
        ...
        tensor = tf.reshape(tensor, [batch_size, tensorlen, self.n_heads, int(new_feature_size)])
    ...
    def _combine_heads(self, tensor):
        ...
        tensor = tf.reshape(tensor, [batch_size, tensorlen, int(new_feature_size)]) # Reshape the Tensor to remove the heads dimension and come back to a Rank-3 tensor
        return tensor
        ##########################################################################################
...
class MultiHeadAttention(Model):
    ...
    def call(self, inputs, mask=None):
        ...
        #if query_antecedent.shape != memory_antecedent.shape:
            #print(q.shape)
            #print(k.shape)
            #print(v.shape)
            #return self.attention_layer.attention_map(q, k, v, mask = mask)[0]
